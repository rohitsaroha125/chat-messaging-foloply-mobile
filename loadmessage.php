...
#sent{margin-top:5px;background:#003f3c;display:inline-block;width:80%;height:auto;
			         border-radius:4px;font-size:17px;color:white;margin-right:1%}
...
$countquery=mysqli_query($connection,"select * from messages where userto='$getid' && userfrom='$msg' || userto='$msg' && userfrom='$getid'") or die(mysqli_error($connection));
$countmessage=mysqli_num_rows($countquery);
if($countmessage==0)
{
	echo "<center><b>Start conversation with ".$fname."</b></center>";
}
$resultquery=mysqli_query($connection,"select * from (select * from messages where userto='$getid' && userfrom='$msg' || userto='$msg' && userfrom='$getid' order by id desc limit 0,$number) tmp order by id asc");
